...
class PrediksiController extends Controller
{
    ...
    private const BETA_DEFAULT = 0.4;
    ...
    {
        $best = ['alpha' => 0.3, 'beta' => 0.4, 'gamma' => 0.1];
        $n = count($data);
        if ($n < 4) {
            return $best;
        }
    ...
    private function processPrediction(string $kelompokId, string $jenisKegiatan, ?Carbon $referenceDate): array
    {
        ...
        // Rata-rata forecast (prediksi jam/menit): F̄ = Σ Ft ÷ n, dengan n = seluruh data (t=1 sampai n).
        // Contoh: 106 data → F̄ = Σ Ft ÷ 106 = 0,3464 jam → 0,3464 × 60 = 20,78 menit.
        // MAPE tetap dihitung dari 20% data testing saja.
        $allForecasts = $result['forecasts'];
        $totalN = count($allForecasts);
        $displayForecast = $totalN > 0 ? max(0.0, array_sum($allForecasts) / $totalN) : $result['nextForecast'];
    ...
    public function generatePrediksiKegiatan(Request $request)
    {
        ...
        foreach ($jenisList as $jenis) {
            ...
            $results[] = [
                'jenis_kegiatan' => $norm,
                'prediksi_jam' => round($pred['nextForecast'], 6), // full precision agar konversi ke menit ≈ 20,78 (F̄ dari seluruh data)
                'tanggal_prediksi' => $tanggalPrediksi->format('Y-m-d'),
    ...
    public function getPrediksiKegiatanByKelompok(Request $request)
    {
        ...
        foreach ($grouped as $norm => $predictions) {
            ...
            $results[] = [
                'jenis_kegiatan' => $norm,
                'prediksi_jam' => round((float) $p->prediksi_jam, 6), // full precision agar tampilan menit ≈ 20,78
                'tanggal_prediksi' => $p->tanggal_prediksi->format('Y-m-d'),
