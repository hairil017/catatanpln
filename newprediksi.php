...
class PrediksiController extends Controller
{
     ...
     * Contoh: 106 record → 85 training (No. 1–85), 21 testing (No. 86–106).
     * Hanya data testing ini yang dipakai untuk MAPE dan rata-rata forecast (total Ft ÷ 21 = 0,4003 jam ≈ 24,02 menit).
     */
    private function getTrainTestSplit(int $n): array
    {
        ...
        $testingCount = max(1, (int) round($n * 0.20)); // persis 20% (106 → 21)
        $trainingCount = $n - $testingCount;
    ...
    private function processPrediction(string $kelompokId, string $jenisKegiatan, ?Carbon $referenceDate): array
    {
        ...
        // MAPE hanya dari data testing (No. 86–106), tidak dari training
        $mape = $this->calculateMAPE($historicalData, $result['forecasts'], $testingStartIdx, $testingCount);

        // Rata-rata forecast = total seluruh Ft testing ÷ 21 (tanpa data lain, tanpa pembulatan sebelum bagi).
        // Target manual: total Ft = 8,406 jam → 8,406 ÷ 21 = 0,4003 jam ≈ 24,02 menit. Konversi ke menit hanya saat tampilan.
        ...
        if ($testingCount > 0 && $testingStartIdx >= 0) {
            ...
            $totalFtTesting = array_sum($testingForecasts);
            $displayForecast = max(0.0, $totalFtTesting / count($testingForecasts)); // presisi penuh, tidak round sebelum ÷
        }
    ...
    public function generatePrediksiKegiatan(Request $request)
    {
        ...
        foreach ($jenisList as $jenis) {
            ...
            $results[] = [
                'jenis_kegiatan' => $norm,
                'prediksi_jam' => round($pred['nextForecast'], 6), // full precision agar konversi ke menit = ±24,91 (bukan 25,2)
                'tanggal_prediksi' => $tanggalPrediksi->format('Y-m-d'),
    ...
    public function getPrediksiKegiatanByKelompok(Request $request)
    {
        ...
        foreach ($grouped as $norm => $predictions) {
            ...
            $results[] = [
                'jenis_kegiatan' => $norm,
                'prediksi_jam' => round((float) $p->prediksi_jam, 6), // full precision agar tampilan menit = ±24,91
                'tanggal_prediksi' => $p->tanggal_prediksi->format('Y-m-d'),
